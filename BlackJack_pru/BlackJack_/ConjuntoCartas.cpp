...
/* Referencia del m�todo barajarCartas():
...
void ConjuntoCartas::barajarCartas() {
    ...
    // Si la lista no est� vac�a, se mete todo lo que tiene la cola
    while (!cartas.empty()) {
    ...
    for (int indiceCarta = n; indiceCarta > 0; indiceCarta--) {
        // Genera un �ndice aleatorio entre 0 e indiceCarta -1
        int indiceAleatorio = rand() % indiceCarta;

        // Mueve las cartas de la cola hasta el �ndice aleatorio
        ...
        // Agrega la carta en el �ndice aleatorio al vector de cartas barajadas
